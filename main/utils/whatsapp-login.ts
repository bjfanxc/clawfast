...
import { getOpenClawDir } from './openclaw-paths'
...
} = nodeRequire(baileysPath)

const QRCodeModule = nodeRequire(join(qrcodeTerminalPath, 'vendor', 'QRCode', 'index.js'))
const QRErrorCorrectLevelModule = nodeRequire(join(qrcodeTerminalPath, 'vendor', 'QRCode', 'QRErrorCorrectLevel.js'))
...
function getAuthDir(accountId: string) {
  return join(homedir(), '.openclaw', 'credentials', 'whatsapp', accountId)
}
...
export class WhatsAppLoginManager extends EventEmitter {
  ...
  private async finishLogin(accountId: string) {
    if (!this.active) {
      return
    }
    await this.stop()
  ...
  async start(accountId = 'whatsapp'): Promise<void> {
    if (this.active && this.accountId === accountId) {
      if (this.qr) {
        this.emit('qr', { qr: await renderQrPngBase64(this.qr), raw: this.qr })
      }
      return
    }
  ...
  private async connectToWhatsApp(accountId: string): Promise<void> {
    ...
    try {
      ...
      try {
        pino = nodeRequire(nodeRequire.resolve('pino', { paths: [baileysPath] }))
      } catch {
        pino = nodeRequire('pino')
      }
        ...
        if (connection === 'close') {
          ...
          if (shouldReconnect && this.active) {
            if (this.retryCount < this.maxRetries) {
              this.retryCount += 1
              setTimeout(() => void this.connectToWhatsApp(accountId), 1000)
            } else {
              this.active = false
            ...
            this.emit('error', error?.message || 'Logged out')
          }
      ...
      if (this.active && this.retryCount < this.maxRetries) {
        this.retryCount += 1
        setTimeout(() => void this.connectToWhatsApp(accountId), 2000)
      } else {
        this.active = false
